  ...
  useEffect(() => {
    ...
    async function loadImages() {
      try {
        ...
        if (!ignore) {
          setErrorMessage(t('galeria_seccion.error_carga'));
        }
      } finally {
        if (!ignore) {
          setIsLoading(false);
        ...
        <div className="mx-auto w-full max-w-[70%]">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.55, ease: 'easeOut' }} className="mb-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            <span className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-4 py-2 text-sm font-bold text-primary">
              ...
              {t('galeria_seccion.etiqueta')}
            </span>
              <h2 className="mt-4 text-4xl font-black text-text-h md:text-5xl">{t('galeria_seccion.titulo')}</h2>
              <p className="mt-4 max-w-2xl text-lg text-text-muted">{t('galeria_seccion.descripcion')}</p>
            </div>

            <div className="rounded-3xl bg-white px-6 py-5 shadow-lg shadow-primary/5">
              <p className="text-sm font-semibold uppercase tracking-[0.3em] text-primary/60">{t('galeria_seccion.fotos_publicadas')}</p>
          ...
          {isLoading ? (
              <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                {Array.from({ length: 3 }).map((_, index) => (
                    <div key={index} className="overflow-hidden rounded-[32px] bg-white shadow-lg shadow-primary/5">
                      <div className="h-72 animate-pulse bg-primary/10" />
              ...
              <div className="rounded-[32px] border border-dashed border-primary/20 bg-white p-10 text-center">
                <p className="text-xl font-bold text-primary">{t('galeria_seccion.vacio_titulo')}</p>
                <p className="mt-2 text-text-muted">{t('galeria_seccion.vacio_descripcion')}</p>
              </div>
          ) : (
              <>
                <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                ...
                <div className="mt-12 text-center">
                  <Link to="/galeria" className="inline-flex cursor-pointer items-center gap-2 rounded-2xl bg-primary px-8 py-4 font-bold text-white shadow-lg shadow-primary/25 transition hover:scale-105 hover:bg-primary/90">
                    {t('galeria_seccion.boton')} <ArrowRight className="h-5 w-5" />
                  </Link>
                </div>
              </>
          )}
