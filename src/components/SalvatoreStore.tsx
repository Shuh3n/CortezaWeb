      ...
      <section id="tienda" className="py-24 bg-neutral-soft overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-20 relative">
            ...
            >
              {t('tienda.titulo')} <span className="text-primary underline decoration-accent/30 decoration-8 underline-offset-8 italic">Salvatore</span>
            </motion.h2>
          ...
          <div className="min-h-[600px] relative">
            {isLoading ? (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 py-20">
                  ...
                  <p className="text-text-muted font-bold tracking-widest uppercase text-sm">{t('tienda.estados.cargando')}</p>
                </div>
            ) : (
                  ...
                  >
                    {paginatedProducts.length > 0 ? (
                        ...
                        <div className="col-span-full py-20 text-center">
                          <p className="text-text-muted font-medium italic">{t('tienda.estados.proximamente')}</p>
                        </div>
                    )}
                  </motion.div>
                </AnimatePresence>
                    ...
                    <div className="md:w-[55%] p-8 md:p-14 overflow-y-auto">
                      ...
                      <div className="flex flex-wrap gap-4 mb-8">
                        <div className="bg-primary/5 border border-primary/10 rounded-[24px] p-5 flex-1 min-w-[150px]">
                          <p className="text-[10px] text-primary font-bold uppercase tracking-widest mb-1 opacity-70">{t('tienda.modal.especificaciones')}</p>
                        ...
                        <div className={`rounded-[24px] p-5 flex-1 min-w-[150px] border ${selectedProduct.stock > 0 ? 'bg-emerald-50 border-emerald-100' : 'bg-red-50 border-red-100'}`}>
                          <p className={`text-[10px] font-bold uppercase tracking-widest mb-1 opacity-70 ${selectedProduct.stock > 0 ? 'text-emerald-700' : 'text-red-700'}`}>{t('tienda.modal.disponibilidad')}</p>
                          ...
                          { t: t('tienda.tipis.tallas.l'), m: '75 x 64 cm', h: '1.10mt' }
                            ...
                            <div key={size.t} className="bg-white/80 backdrop-blur-sm p-5 rounded-2xl flex justify-between items-center shadow-sm border border-white group hover:bg-white hover:scale-[1.02] transition-all duration-300">
                              <div>
                                ...
                                <p className="text-xs text-text-muted font-semibold tracking-wide uppercase">{t('tienda.tipis.altura')}: {size.h}</p>
                              </div>
                  ...
                  <table className="w-full text-left">
                    ...
                    <tr className="text-primary border-b border-slate-100">
                      {tshirtColumns.map((col, idx) => (
                          <th key={idx} className="py-4 px-4">{col}</th>
                      ))}
                    </tr>
                    </thead>
          ...
          <div className="mt-24 grid lg:grid-cols-2 gap-8">
            <div className="bg-primary rounded-[48px] p-12 text-white relative overflow-hidden shadow-2xl shadow-primary/30">
              <div className="relative z-10">
                ...
                <h3 className="text-3xl font-bold mb-6">{t('tienda.banner.titulo')}</h3>
